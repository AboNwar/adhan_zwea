...
class _PrayerTimesScreenState extends State<PrayerTimesScreen> {
    ...
    setState(() {
      ...
      if (cached != null && cached.isNotEmpty) {
        try {
          ...
          prayerTimes = json.map((k, v) => MapEntry(k, v.toString()));
        } catch (_) {
          // ignore cache errors
        }
      }
  ...
  Future<void> _loadPrayerTimes() async {
    ...
    try {
      ...
      final prefs = await SharedPreferences.getInstance();
      await prefs.setString('prayer_times_cache', jsonEncode(map));
      try {
  ...
  // حساب نافذة الصامت لصلاة محددة: تبدأ قبل انتهاء الإقامة بـ5 دقائق وتنتهي بعدها بـ10 دقائق
